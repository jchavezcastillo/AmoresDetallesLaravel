<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="es" lang="es" class="ve-animate">
<head></head>
<body>

	@include('visitors.master-visitors')

	<div class="container">

		<div class="col-xs-12 col-sm-12 col-md-2 col-md-lg-2">
			<ul class="padding-bottom-2 menu-view">
				<li class="margin-left-none"><span class="title-category">ACCESORIOS Y DECORACIÓN</span></li>
				<a href="{!! route('visitors.nina') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Niña
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.nino') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Niño
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.ella') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Ella
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.el') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Él
						</div>
					</li>
				</a>
				<li><br></li>
				<a href="{!! route('visitors.hogar') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Hogar
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.oficina') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Oficina
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.escuela') !!}/#menu-ancla">
					<li class="subrayar">
						<div class="input-group">
						  &#8226;Escuela
						</div>
					</li>
				</a>
				<a href="{!! route('visitors.galeria') !!}/#menu-ancla">
					<li>
						<div class="input-group">
						  &#8226;Galería
						</div>
					</li>
				</a>
			</ul>
		</div>

		<div class="col-xs-12 col-sm-12 col-md-10 col-md-lg-10">
			<div class="row"> 
					
				<div class="col-xs-12 col-sm-12 col-md-4 col-md-lg-4"> 
					<div class="thumbnail"> 
						<img alt="100%x200" data-src="holder.js/100%x200" style="height: 200px; width: 100%; display: block;" src="data:image/svg+xml;base64," data-holder-rendered="true"> 
						<div class="caption"> 
							<h3>Nombre del producto ...</h3> 
								<p>Descripción ....</p> 
								<p><a href="#" class="btn btn-primary" role="button">Solicitud de pedido</a></p> 
						</div> 
					</div> 
				</div> 

				<div class="col-xs-12 col-sm-12 col-md-4 col-md-lg-4"> 
					<div class="thumbnail"> 
						<img alt="100%x200" data-src="holder.js/100%x200" style="height: 200px; width: 100%; display: block;" src="data:image/svg+xml;base64," data-holder-rendered="true"> 
						<div class="caption"> 
							<h3>Nombre del producto ...</h3> 
								<p>Descripción ....</p> 
								<p><a href="#" class="btn btn-primary" role="button">Solicitud de pedido</a></p> 
						</div> 
					</div> 
				</div> 

				<div class="col-xs-12 col-sm-12 col-md-4 col-md-lg-4"> 
					<div class="thumbnail"> 
						<img alt="100%x200" data-src="holder.js/100%x200" style="height: 200px; width: 100%; display: block;" src="data:image/svg+xml;base64," data-holder-rendered="true"> 
						<div class="caption"> 
							<h3>Nombre del producto ...</h3> 
								<p>Descripción ....</p> 
								<p><a href="#" class="btn btn-primary" role="button">Solicitud de pedido</a></p> 
						</div> 
					</div> 
				</div> 

			</div>
		</div>
				<!--Más producto-->
	</div>
				
	
</body>
</html>
